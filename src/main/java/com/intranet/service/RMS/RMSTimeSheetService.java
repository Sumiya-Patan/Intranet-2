...
public class RMSTimeSheetService {
    ...
    public TimeSheetSummaryResponseDTO getSummary(
        ...
        List<TimeSheet> timeSheets = userId == null
                ? timeSheetRepository.findByWorkDateBetweenWithWeekInfoAndEntries(startDate, endDate)
                : timeSheetRepository.findByUserIdAndWorkDateBetweenWithEntries(userId, startDate, endDate);
        ...
        for (Map.Entry<Long, List<TimeSheet>> entry : timeSheetsByUser.entrySet()) {
            ...
            BigDecimal billableHours = sumEntryHours(userEntries, TimeSheetEntry::isBillable);
            ...
            BigDecimal nonBillableHours = sumEntryHours(userEntries, entry2 -> !entry2.isBillable() && !(entry2.getProjectId() != null && internalProjectIds.contains(entry2.getProjectId())));
            ...
            BigDecimal totalHours = entryHours.add(autoGeneratedHours);
        ...
        List<ProjectUtilizationDTO> projects = buildProjectUtilization(entries, overallTotal, overallPlanned, internalProjectIds);
    ...
    public List<ResourceSummaryMinimalDTO> getAllResourceSummaries(LocalDate startDate, LocalDate endDate) {
        return getSummary(null, startDate, endDate).getResourceSummaries().stream()
    ...
    private List<ProjectUtilizationDTO> buildProjectUtilization(List<TimeSheetEntry> entries,
                                                                 BigDecimal totalHours,
                                                                 BigDecimal plannedCapacity,
                                                                 Set<Long> internalProjectIds) {
        Map<Long, List<TimeSheetEntry>> groupedByProject = entries.stream()
        ...
        return groupedByProject.entrySet().stream()
                .map(projectEntry -> {
                    ...
                    List<TimeSheetEntry> projectEntries = projectEntry.getValue();
                    BigDecimal actualHours = sumEntryHours(projectEntries, entry -> true);
                    BigDecimal billable = sumEntryHours(projectEntries, TimeSheetEntry::isBillable);
                    BigDecimal nonBillable = sumEntryHours(projectEntries, entry -> !entry.isBillable());
                    BigDecimal internal = sumEntryHours(projectEntries, entry -> internalProjectIds.contains(entry.getProjectId()));
                    BigDecimal plannedHours = totalHours.compareTo(BigDecimal.ZERO) == 0
                            ? actualHours
                            : actualHours.multiply(plannedCapacity).divide(totalHours, 2, RoundingMode.HALF_UP);
                    double util = safePercentage(actualHours, plannedHours);
                    String health = util >= 90 ? "Healthy" : util >= 70 ? "At Risk" : "Critical";
                    String breach = util < 70 ? "BREACH" : "NONE";
                    String healthSignal = String.format("%s (%.2f%%)", health, util);
                    ...
                    return ProjectUtilizationDTO.builder()
                            ...
                            .utilizationPercentage(round(util))
                ...
                .sorted(Comparator.comparing(ProjectUtilizationDTO::getProjectId))
                .collect(Collectors.toList());
    }
    ...
    public MonthlySummaryResponseDTO getMonthlySummary(LocalDate startDate, LocalDate endDate) {
        ...
        TimeSheetSummaryResponseDTO baseSummary = getSummary(null, startDate, endDate);
